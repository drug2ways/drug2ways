...
for l in `seq $lmin $lmax`; do
	...
	--graph ${pkg_path}/networks/data/custom_network.tsv \
	--sources ${pkg_path}/validation/data/source_nodes_custom.tsv \
	--targets ${pkg_path}/validation/data/target_nodes_custom.tsv \
	--fmt tsv \
	--lmax ${l} \
	--output ${output_path} \
	--name custom \
